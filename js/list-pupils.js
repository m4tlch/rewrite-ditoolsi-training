(function ($) {
    ...
    Drupal.behaviors.ditoolsiTrainingDayPicker = {
        attach: function (context, setting) {
            ...
            $('form#list-pupils .selecting-date.day').once('ditoolsiTrainingDayPicker', function () {
                var $container = $(this);
    ...
    Drupal.behaviors.ditoolsiTrainingSwitchPupil = {
        attach: function () {
            $('.select-pupil-lesson-switch').bind('change', function (e) {

                var optionSelected = $("option:selected", this),
                    valueSelected = this.value,
                    uI = $(this).attr('user_id'),
                    pathname = window.location.pathname.split("/"),
                    linkCourse = pathname[pathname.length - 2];
                $.ajax({
                    type: 'POST',
                    url: Drupal.settings.basePath + '?q=courses/pupil-switch',
                    data: {
                        cID: linkCourse,
                        uID: uI,
                        lesson_switch: valueSelected,
                    },
                    dataType: 'json',
                    async: false,
                    success: function (response) {
                        $('.list-pupils-status').empty();
                        $('.list-pupils-status').append(response.status);
                    }
                });
            });
        }
    ...
    Drupal.behaviors.changeLessonLink = {
        attach: function () {
            $('.user-passed-lesson-list').on('change', function (e) {
                var lessonId = $("option:selected", this).val();
                var courseId = $(this).data('cid');
                var userId = $(this).data('uid');
                var link = "/course/" + courseId + "/" + lessonId + "/" + userId + "/view-history";
                var $lessonLink = $(this).parents('td').find('.lesson-link');
                $lessonLink.attr('href', link);
            });
        }
    };
