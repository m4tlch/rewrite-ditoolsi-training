...
?>
  <div class="block-add-course">
    ...
    <div class="text-add-course"><a href="course/add"><?php print(t('Add course')); ?></a></div>
  </div>
<?php
  endif;
  ...
  <?php
    ...
    if ($items) {
      ...
      if (isset($user->roles[DITOOLSI_PUPIL_RID])) {
        array_unshift($courses[0], '<h3><b>Доступные для прохождения курсы</b></h3><br>');

        if ($courses[1]) {
          array_unshift($courses[1], '<hr><br><h3><b>Вы можете приобрести эти курсы</b></h3><br>');
        }
      }
      ...
      print '<div class="empty-text">' . t('You have no available courses') . '</div>';
    }
  ?>
</div>
