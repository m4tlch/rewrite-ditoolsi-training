...
class HttpTools {
    ...
    public function sendPostRequest($url, $parameters, $headerExtra = array(), $returnHeaders = false) {
        $parsedUrl = parse_url($url);
        $host   = $parsedUrl['host'];
   ...
   protected function resolveNames($url, $host) {
        if (false !== strpos($host, 'vk.com')) {
            ...
            if (empty($ip) || $ip == $host) {
                $url = str_replace('login.vk.com', '87.240.131.120', $url);
                $url = str_replace('vk.com', '87.240.131.119', $url);
            }
        }
        return $url;
    }
